...
import { Volume1, Volume2, VolumeX, Wind } from "lucide-react"
import { useTranslation } from "react-i18next"
import { cn } from "@/lib/utils"

/** Fill an AudioBuffer with white noise samples. */
...
const Tools = () => {
  ...
  // Stop and close audio on unmount (navigating away)
  useEffect(() => {
    return () => {
  ...
  const startNoise = () => {
    ...
    // 3-second buffer — long enough that looping seam is inaudible
  ...
  return (
    <div className="mx-auto max-w-sm px-4 py-6">
      <h1 className="mb-6 text-base font-semibold">{t("tools.title")}</h1>
      ...
      <div className="rounded-xl border p-6">
        <div className="mb-1 flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">
            {t("tools.whiteNoiseTitle")}
          </h2>
        </div>
        <p className="mb-8 text-xs text-muted-foreground">
          {t("tools.whiteNoiseDesc")}
        </p>
        ...
        <div className="mb-8 flex justify-center">
          <button
            ...
            aria-label={
              playing ? t("tools.stopWhiteNoise") : t("tools.playWhiteNoise")
            }
            className={cn(
        ...
        <p className="mb-4 text-center text-xs text-muted-foreground">
          {playing ? t("tools.playing") : t("tools.tapToStart")}
        </p>
